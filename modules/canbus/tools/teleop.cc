...
class Teleop {
 public:
  ...
  //读取键盘操作，设置control_command_下的成员数值，并且将新的数值打印在终端
  ...
  void GetPadMessage(PadMessage *pad_msg, int32_t int_action) {
    ...
    switch (int_action) {
      ...
      case 1:
        ...
        //加入一条控制指令
        control_command_.set_engine_on_off(true);
...
int main(int32_t argc, char **argv) {
  ...
  std::cout << "ros node creat OK" << std::endl;

  apollo::common::adapter::AdapterManagerConfig config;
  //设置各种ros的信息，包括subscribe和publish
  config.set_is_ros(true);
  {
    auto *sub_config = config.add_config();
    //publish only，这是teleop发出的指令，名字叫control command
