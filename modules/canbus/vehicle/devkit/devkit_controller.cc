...
ErrorCode DevkitController::Init(
  ...
  if (iecu_control_distributed_505_ == nullptr) {
     AERROR << "Iecucontroldistributed505 does not exist in the DevkitMessageManager!";
     return ErrorCode::CANBUS_ERROR;
  }
  ...
  if (iecu_control_flag_501_ == nullptr) {
     AERROR << "Iecucontrolflag501 does not exist in the DevkitMessageManager!";
     return ErrorCode::CANBUS_ERROR;
  }
  ...
  if (iecu_control_ibc_503_ == nullptr) {
     AERROR << "Iecucontrolibc503 does not exist in the DevkitMessageManager!";
     return ErrorCode::CANBUS_ERROR;
  }
  ...
  if (iecu_control_power_504_ == nullptr) {
     AERROR << "Iecucontrolpower504 does not exist in the DevkitMessageManager!";
     return ErrorCode::CANBUS_ERROR;
  }
  ...
  if (iecu_control_steering_502_ == nullptr) {
     AERROR << "Iecucontrolsteering502 does not exist in the DevkitMessageManager!";
     return ErrorCode::CANBUS_ERROR;
  }
...
Chassis DevkitController::chassis() {
 ...
 if (!chassis_detail.has_devkit()) {
  ...
  //TSY devkit = chassis_detail.devkit();
  //4 Devkit:devkit 没有发动机转速信号
  //5 wheel spd 0x305或者0x10-0x40
  if (chassis_detail.devkit().has_vcu_vehicle_status_3_305()) {
    if (chassis_detail.devkit().vcu_vehicle_status_3_305().has_rpm_fl_rpt()) {
      chassis_.mutable_wheel_speed()->set_wheel_spd_fl(
          chassis_detail.devkit().vcu_vehicle_status_3_305().rpm_fl_rpt());
    }
    if (chassis_detail.devkit().vcu_vehicle_status_3_305().has_rpm_fr_rpt()) {
      chassis_.mutable_wheel_speed()->set_wheel_spd_fr(
          chassis_detail.devkit().vcu_vehicle_status_3_305().rpm_fr_rpt());
    }
    if (chassis_detail.devkit().vcu_vehicle_status_3_305().has_rpm_rl_rpt()) {
      chassis_.mutable_wheel_speed()->set_wheel_spd_rl(
          chassis_detail.devkit().vcu_vehicle_status_3_305().rpm_rl_rpt());
    }
    if (chassis_detail.devkit().vcu_vehicle_status_3_305().has_rpm_rr_rpt()) {
      chassis_.mutable_wheel_speed()->set_wheel_spd_rr(
          chassis_detail.devkit().vcu_vehicle_status_3_305().rpm_rr_rpt());
    }
  }
  //6 speed_mps 这个是米每秒的车速  0x304
  if (chassis_detail.devkit().has_vcu_vehicle_status_2_304() && chassis_detail.devkit().vcu_vehicle_status_2_304().has_vehicle_speed()) {
    chassis_.set_speed_mps(
        static_cast<float>(chassis_detail.devkit().vcu_vehicle_status_2_304().vehicle_speed()));
  } else {
  ...
  // 9 throttle 0x304
  if (chassis_detail.devkit().has_vcu_vehicle_status_2_304() &&
      chassis_detail.devkit().vcu_vehicle_status_2_304().has_throttle_rpt()) {
    chassis_.set_throttle_percentage(static_cast<float>(
        chassis_detail.devkit().vcu_vehicle_status_2_304().throttle_rpt()));
  } else {
    chassis_.set_throttle_percentage(0);
  }
  // 10 brake 0x304
  if (chassis_detail.devkit().has_vcu_vehicle_status_2_304() &&
      chassis_detail.devkit().vcu_vehicle_status_2_304().has_vehicle_brake_pressure()) {
    chassis_.set_brake_percentage(static_cast<float>(
        chassis_detail.devkit().vcu_vehicle_status_2_304().vehicle_brake_pressure()));
  } else {
    chassis_.set_brake_percentage(0);
  }
  /*  下面是原来用0x142的传感器反馈的，这里为了测试就改用0x304了
  if (devkit.has_dbsf_status_142() &&
      devkit.dbsf_status_142().has_dbsf_hp_pressure()) {
    chassis_.set_brake_percentage(static_cast<float>(
        devkit.dbsf_status_142().has_dbsf_hp_pressure()));
  } else {
    chassis_.set_brake_percentage(0);
  ...
  */
  ...
  if (chassis_detail.devkit().has_vcu_vehicle_status_1_303() &&
      chassis_detail.devkit().vcu_vehicle_status_1_303().has_gear_rpt()) {
    Chassis::GearPosition gear_pos = Chassis::GEAR_INVALID;

    if (chassis_detail.devkit().vcu_vehicle_status_1_303().gear_rpt() ==
  ...
  // 12 steering 0x304
  if (chassis_detail.devkit().has_vcu_vehicle_status_2_304() &&
      chassis_detail.devkit().vcu_vehicle_status_2_304().has_f_tire_angle()) {
    chassis_.set_steering_percentage(static_cast<float>(
        chassis_detail.devkit().vcu_vehicle_status_2_304().f_tire_angle() *
  ...
  // 13 parking brake
  if (chassis_detail.devkit().has_vcu_vehicle_status_1_303() &&
      chassis_detail.devkit().vcu_vehicle_status_1_303().has_epb_rpt()) {
    if (chassis_detail.devkit().vcu_vehicle_status_1_303().epb_rpt() ==
...
// devkit default, -470 ~ 470, left:+, right:-
...
bool DevkitController::CheckChassisError() {
  ...
  // steer fault
  if (devkit.has_vcu_vehicle_diagnosis_301()) {
    if (Vcu_vehicle_diagnosis_301::FSTEERING_STATE_FSTR_FAULT ==
        devkit.vcu_vehicle_diagnosis_301().fsteering_state()) {
      return true;
    }
  }
  // drive fault
  if (devkit.has_vcu_vehicle_diagnosis_301()) {
    if (Vcu_vehicle_diagnosis_301::FLMOTOR_STATE_FL_FAULT ==
       devkit.vcu_vehicle_diagnosis_301().flmotor_state()) {
      return true;
    }
    if (Vcu_vehicle_diagnosis_301::FRMOTOR_STATE_FR_FAULT ==
       devkit.vcu_vehicle_diagnosis_301().frmotor_state()) {
      return true;
    }
    if (Vcu_vehicle_diagnosis_301::RLMOTOR_STATE_RL_FAULT ==
       devkit.vcu_vehicle_diagnosis_301().rlmotor_state()) {
      return true;
    }
    if (Vcu_vehicle_diagnosis_301::RRMOTOR_STATE_RR_FAULT ==
       devkit.vcu_vehicle_diagnosis_301().rrmotor_state()) {
      return true;
    }
  }
  // brake fault
  if (devkit.has_vcu_vehicle_diagnosis_301()) {
    if (Vcu_vehicle_diagnosis_301::DBSF_STATE_DBSF_FAULT ==
        devkit.vcu_vehicle_diagnosis_301().dbsf_state()) {
      return true;
    }
  }
  // gear fault
  ...
  */
  ...
  if (devkit.has_vcu_vehicle_diagnosis_301()) {
    if (Vcu_vehicle_diagnosis_301::EPB_STATE_EPB_FAULT ==
        devkit.vcu_vehicle_diagnosis_301().epb_state()) {
      return true;
    }
  }
  return false;
...
void DevkitController::SecurityDogThreadFunc() {
  ...
  while (can_sender_->IsRunning()) {
    ...
    if (elapsed < default_period) {
      ...
      AERROR
          << "Too much time consumption in DevkitController looping process:"
          << elapsed.count();
    }
  }
}
