...
void Mcufldrivemotorfeedbackmsg60::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_devkit()->mutable_mcufl_drive_motor_feedback_msg_60()->set_fl_reserved_1(fl_reserved_1(bytes, length));
  chassis->mutable_devkit()->mutable_mcufl_drive_motor_feedback_msg_60()->set_fl_mcu_motor_error_grade(fl_mcu_motor_error_grade(bytes, length));
  chassis->mutable_devkit()->mutable_mcufl_drive_motor_feedback_msg_60()->set_fl_energy_recovery_state(fl_energy_recovery_state(bytes, length));
  chassis->mutable_devkit()->mutable_mcufl_drive_motor_feedback_msg_60()->set_fl_controltemp(fl_controltemp(bytes, length));
  chassis->mutable_devkit()->mutable_mcufl_drive_motor_feedback_msg_60()->set_fl_idc(fl_idc(bytes, length));
  chassis->mutable_devkit()->mutable_mcufl_drive_motor_feedback_msg_60()->set_fl_udc(fl_udc(bytes, length));
}
