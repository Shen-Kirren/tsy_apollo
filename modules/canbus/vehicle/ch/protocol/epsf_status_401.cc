...
void Epsfstatus401::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_ch()->mutable_epsf_status_401()->set_epsf_check_sum(epsf_check_sum(bytes, length));
  chassis->mutable_ch()->mutable_epsf_status_401()->set_epsf_reserved2(epsf_reserved2(bytes, length));
  chassis->mutable_ch()->mutable_epsf_status_401()->set_epsf_angle_in_middle_calib(epsf_angle_in_middle_calib(bytes, length));
  chassis->mutable_ch()->mutable_epsf_status_401()->set_low_epsf_angle(low_epsf_angle(bytes, length));
  chassis->mutable_ch()->mutable_epsf_status_401()->set_high_epsf_angle(high_epsf_angle(bytes, length));
  chassis->mutable_ch()->mutable_epsf_status_401()->set_epsf_reserved1(epsf_reserved1(bytes, length));
  chassis->mutable_ch()->mutable_epsf_status_401()->set_epsf_steering_wheel_torque_value(epsf_steering_wheel_torque_value(bytes, length));
  chassis->mutable_ch()->mutable_epsf_status_401()->set_epsf_work_mode_status(epsf_work_mode_status(bytes, length));
  //tsy_controller.cc中CheckReponse中要求的代码
  //chassis->mutable_check_response()->set_is_eps_online(epsf_work_mode_status(bytes, length) ==32);
  //问题记录，2021.2.25发现这个信号常为1，代码中直接将信号的状态赋值了true，所以再次记录，将online()内的逻辑改为无错误码或者是角度控制模式0x20。
}
