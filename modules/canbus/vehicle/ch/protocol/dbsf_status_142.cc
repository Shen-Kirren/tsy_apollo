...
void Dbsfstatus142::Parse(const std::uint8_t* bytes, int32_t length,
  ...
  chassis->mutable_ch()->mutable_dbsf_status_142()->set_dbsf_reserved_2(dbsf_reserved_2(bytes, length));
  chassis->mutable_ch()->mutable_dbsf_status_142()->set_dbsf_fault_code(dbsf_fault_code(bytes, length));
  chassis->mutable_ch()->mutable_dbsf_status_142()->set_dbsf_hp_pressure(dbsf_hp_pressure(bytes, length));
  chassis->mutable_ch()->mutable_dbsf_status_142()->set_dbsf_reserved_1(dbsf_reserved_1(bytes, length));
  chassis->mutable_ch()->mutable_dbsf_status_142()->set_dbsf_system_status(dbsf_system_status(bytes, length));
  //tsy_controller.cc中CheckReponse中要求的代码
  //chassis->mutable_check_response()->set_is_esp_online(dbsf_fault_code(bytes, length) == 0);
}
